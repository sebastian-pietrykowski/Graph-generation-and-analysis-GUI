...
int bfs(graph_t graph, int start_vertex_number) {
    ...
    int* visited = malloc(graph->no_vertexes * sizeof(*visited)); /* Memory allocation for vertexes that have already been visited */
...
void free_bfs ( FIFO_t fifo, int * vertexes ) {
    free(fifo->vertexes);
    free(fifo);
    free(vertexes);
}
