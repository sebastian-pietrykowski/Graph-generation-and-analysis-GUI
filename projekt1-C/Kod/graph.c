 ...
 *
 *  then we add a number that is the product of "(graph->no_vertexes - 2 * graph->rows )" because this is the number of vertices that are not the most distant vertices from the center of the graph - lying on its outer sides
 *  and "4" ( because this vertices can be connected to up to four other vertices
 */
  if (temp != (4 * 2 + (graph->columns - 2) * 2 * 3 + (graph->rows - 2) * 2 * 3 + (graph->no_vertexes - 2 * graph->rows) * 4 ))
     return 0;
   else
     return 1;
}
